...
mod gpui_form_tests {
    ...
    fn test_real_world_common_v_read_pattern() {
        let tokens = quote! {
            ...
            pub struct CommonVRead {
                #[cfg_attr(feature = "ui", gpui_form(shape = crate::NumericShape))]
                #[cfg_attr(feature = "validation", koruma(newtype))]
    ...
    fn test_shape_override_keeps_full_type_path() {
        ...
        assert!(
            compact.contains(
                "FieldVariant::component(\"amount\",::gpui_form::schema::registry::RustType::new_unchecked(\"rust_decimal::Decimal\"),if<<crate::NumericShape<rust_decimal::Decimal>as::gpui_form::runtime::shape::ComponentShape>::ValueStoragePolicyas::gpui_form::runtime::shape::ComponentValueStoragePolicy>::REQUIRES_VALUE"
            ),
    ...
    fn test_default_uses_into_conversion() {
        let tokens = quote! {
            #[derive(GpuiForm)]
            struct TestForm {
                #[gpui_form(shape = crate::Input, default = "test@example.com")]
                email: String,
            }
        };
    ...
    fn test_component_shape_generates_shape_based_state_and_constructor() {
        let tokens = quote! {
            #[derive(GpuiForm)]
            struct TestForm {
                #[gpui_form(shape = crate::ui::BioInput)]
                bio: String,
            }
        };
        ...
        assert!(
            compact.contains(
                "FieldVariant::component(\"bio\",::gpui_form::schema::registry::RustType::new_unchecked(\"String\"),if<<crate::ui::BioInputas::gpui_form::runtime::shape::ComponentShape>::ValueStoragePolicy"
            ),
        ...
        assert!(
            compact.contains(
                "FieldComponentVariant::new(::gpui_form::schema::registry::RustPath::new_unchecked(\"crate::ui::BioInput\"))"
            ),
    ...
    fn test_component_shape_rejects_field_level_field_suffix() {
        let tokens = quote! {
            #[derive(GpuiForm)]
            struct TestForm {
                #[gpui_form(shape = crate::state::TagsState, field_suffix = "tags")]
                labels: Vec<String>,
            }
        };
    ...
    fn test_component_shape_generates_fields() {
        let tokens = quote! {
            #[derive(GpuiForm)]
            struct TestForm {
                #[gpui_form(shape = gpui_form_collection::input::Input::<_>, default = "test@example.com")]
                email: String,
    ...
    fn test_component_shape_rejects_duplicate_component_expression() {
        let tokens = quote! {
            ...
            struct TestForm {
                #[gpui_form(
                    shape = gpui_form_collection::input::Input::<_>,
                    shape = crate::Input
                )]
                email: String,
            }
        ...
        assert!(
            compact.contains("duplicate`shape`option"),
    ...
    fn test_field_variant_metadata_records_form_value_holder_conversion_shape() {
        ...
        assert!(
            compact.contains(
                "FieldVariant::component(\"account_no\",::gpui_form::schema::registry::RustType::new_unchecked(\"crate::types::AccountCode\"),if<<crate::Inputas::gpui_form::runtime::shape::ComponentShape>::ValueStoragePolicy"
            ),
    ...
    fn test_component_shape_rejects_field_value_storage_override() {
        let tokens = quote! {
            #[derive(GpuiForm)]
            struct TestForm {
                #[gpui_form(shape = gpui_form_collection::switch::Switch, value_storage = direct)]
                enabled: bool,
            }
        };
    ...
    fn test_component_shape_rejects_unknown_infinite_select_helper() {
        let tokens = quote! {
            ...
            struct TestForm {
                #[gpui_form(
                    shape = gpui_form_component::infinite_select::InfiniteSelect::<_>,
    ...
    fn test_component_shape_rejects_invalid_field_suffix() {
        let tokens = quote! {
            #[derive(GpuiForm)]
            struct TestForm {
                #[gpui_form(shape = crate::Input, field_suffix = "input-field")]
                name: String,
            }
        };
