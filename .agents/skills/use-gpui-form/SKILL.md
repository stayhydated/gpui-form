...
- For value-bound custom widgets, implement `gpui_form_component::custom::CustomComponentValueBinding<T>` on the shape and use `.value_binding()` in the `component = Shape` expression when the shape does not already publish `VALUE_BINDING`; external wrappers map native events to `FormValueEvent<T>`, while owned states can emit `FormValueEvent<T>` directly and mark the shape with `OwnedCustomComponentValueBinding<T>`.
