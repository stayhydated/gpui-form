...
- For value-bound custom widgets, implement `gpui_form_component::custom::CustomComponentValueAdapter<T>` on the shape and use `.value_binding()` in the `component = Shape` expression when the shape does not already publish `VALUE_BINDING`; the adapter seeds component state with `seed_value_binding_state` and maps native events to `ValueBindingChange<T>`.
